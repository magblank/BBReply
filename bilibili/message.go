...
type UnreadPrivateMessage struct {
	UnfollowUnread int `json:"unfollow_unread"` // UP未关注用户未读私信数
	FollowUnread   int `json:"follow_unread"`   // UP已关注用户未读私信数
...
func SendPrivateMessageText(senderUid, receiverId json.Number, content string) (int, string, error) {
	return std.SendPrivateMessageText(senderUid, receiverId, content)
}
func (c *Client) SendPrivateMessageText(senderUid, receiverId json.Number, content string) (int, string, error) {
	...
	resp, err := c.resty().R().SetHeader("Content-Type", "application/x-www-form-urlencoded").SetQueryParams(map[string]string{
		"msg[sender_uid]":    string(senderUid),
...
type NewSessionMessages struct {
	SessionList []struct {
		TalkerID    int         `json:"talker_id"`
		SessionType int         `json:"session_type"`
		AtSeqno     int         `json:"at_seqno"`
		TopTs       int         `json:"top_ts"`
		GroupName   string      `json:"group_name"`
		...
		IsFollow    int         `json:"is_follow"`
		IsDnd       int         `json:"is_dnd"`
		AckSeqno    json.Number `json:"ack_seqno"`
		AckTs       json.Number `json:"ack_ts"`
		SessionTs   json.Number `json:"session_ts"`
		UnreadCount int         `json:"unread_count"`
		LastMsg     struct {
			SenderUID      json.Number `json:"sender_uid"` //发送者uid
			ReceiverType   int         `json:"receiver_type"`
			ReceiverID     json.Number `json:"receiver_id"` //接受者uid
			MsgType        int         `json:"msg_type"`
			Content        string      `json:"content"`
			MsgSeqno       json.Number `json:"msg_seqno"`
			Timestamp      int         `json:"timestamp"`
			AtUids         interface{} `json:"at_uids"`
			MsgKey         json.Number `json:"msg_key"`
			MsgStatus      int         `json:"msg_status"`
			NotifyCode     string      `json:"notify_code"`
			NewFaceVersion int         `json:"new_face_version"`
		} `json:"last_msg"`
		GroupType         int         `json:"group_type"`
		CanFold           int         `json:"can_fold"`
		Status            int         `json:"status"`
		...
		LiveStatus        int         `json:"live_status"`
		BizMsgUnreadCount int         `json:"biz_msg_unread_count"`
		UserLabel         interface{} `json:"user_label"`
	} `json:"session_list"`
	HasMore             int  `json:"has_more"`
	AntiDisturbCleaning bool `json:"anti_disturb_cleaning"`
	IsAddressListEmpty  int  `json:"is_address_list_empty"`
...
// GetNewSessionMessages 获取新的消息记录
func GetNewSessionMessages() (*NewSessionMessages, error) {
	return std.GetNewSessionMessages()
}
func (c *Client) GetNewSessionMessages() (*NewSessionMessages, error) {
	resp, err := c.resty().R().SetQueryParams(map[string]string{
		"begin_ts": strconv.FormatInt(time.Now().UnixMilli(), 10),
		"build":    "0",
		"mobi_app": "web",
	}).Get("https://api.vc.bilibili.com/session_svr/v1/session_svr/new_sessions")
	if err != nil {
		return nil, err
	}
	data, err := getRespData(resp, "获取新的消息记录")
	if err != nil {
		return nil, err
	}
	var newSessionMessages *NewSessionMessages
	err = json.Unmarshal(data, &newSessionMessages)
	if err != nil {
		return nil, err
	}
	return newSessionMessages, errors.WithStack(err)
}
